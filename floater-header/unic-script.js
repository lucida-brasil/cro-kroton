var floaterTeste = {
  ...
  createMarkup: function() {
    ...
    var zap = '<div class="wa-floater-container"><div class="wa-floater-header"><span class="circle"><img alt="" src="data:image/png;base64," alt="Logo unic"></span><div class="txt"><strong>unic</strong><span>Online</span></div></div><div class="wa-floater-messages"><div class="wa-floater-message"><p>Olá!<br>Para prosseguir com o atendimento, vamos precisar de algumas informações:</p></div><div class="wa-floater-fields"><label><span>Qual o seu nome completo?*</span><input type="text" name="nome-completo" id="nome-completo"></label><label><span>Qual o número do seu celular?*</span><input type="text" name="telefone-celular" id="telefone-celular"></label><label><input type="checkbox" name="consentimento" id="consentimento"><span>Eu concordo em receber contato no WhatsApp.</span></label><button>Iniciar Conversa</button></div></div></div>';
    parser = new DOMParser();
    ...
    w.id = 'funil_widget';
    w.setAttribute('brand', 'unic');
    w.setAttribute('type', 'Graduação');
    w.setAttribute('color', '#F5821F');
    w.setAttribute('domaingraduation', 'https://www.unic.com.br/inscricao');
  ...
  bind: function() {
    ...
    init.addEventListener('click', function() {
      _this.sendEvent('unic:teste', 'floater:inicio', 'clique:botao');
    ...
    agora.addEventListener('click', function() {
      _this.sendEvent('unic:teste', 'floater:widget-inscreva-se', 'clique:botao');
      _this.hA();
      _this.hM();
      _this.cS('close');

      window.location.href = 'https://www.unic.com.br/inscricao?origem=floater';
    });

    var ctc = document.querySelector('.floater-container button.floater-call-me');
    ctc.addEventListener('click', function() {
      _this.sendEvent('unic:teste', 'floater:ctc', 'clique:botao');
      _this.hA();
      _this.hM();
      _this.cS('close');
      document.querySelector('iframe.callme').src = 'https://bluecore-kroton-ctc.azurewebsites.net/embed/unic';
    ...
    chatbot.addEventListener('click', function() {
      _this.sendEvent('unic:teste', 'floater:chatbot', 'clique:botao');
      _this.hA();
      _this.hM();
      _this.cS('close');
    ...
    wap.addEventListener('click', function() {
      _this.sendEvent('unic:teste', 'floater:whatsapp', 'clique:botao');
      _this.hA();
      _this.hM();
      _this.cS('close');
    ...
    wapfields.forEach(function(el) {
      this.addEventListener('change', function() {
        _this.sendEvent('unic:teste', 'floater:whatsapp', 'preencheu:' + el.name);
      })
    })

    var wapsend = document.querySelector('.wa-floater-container button');
    wapsend.addEventListener('click', function() {
      _this.sendEvent('unic:teste', 'floater:whatsapp', 'clique:iniciar-conversa');
  ...
  autoStart: function() {
    ...
    setTimeout(function() {
      if (_this.openedOnce == false) {
        _this.sendEvent('unic:teste', 'floater:inicio', 'auto');
...
var headerTeste = {
  ...
  createButton: function() {
    ...
    this.but.setAttribute('href', 'https://www.unic.com.br/inscricao?origem=header');
  ...
  insertButton: function(mob = false) {
    if (mob) {
      ...
      if (document.querySelectorAll('.btn-header-inscreva-se').length === 0) {
        document.querySelector('.unic_ > .second-menu').appendChild(cont);
      }
    } else {
      if (document.querySelectorAll('.btn-header-inscreva-se').length === 0) {
  ...
  bind: function() {
    ...
    document.querySelector('.btn-header-inscreva-se').addEventListener('click', function() {
      _this.sendEvent('unic:teste', 'header', 'clique:inscreva-se');
    })

    document.querySelector('.lnk-header-acompanhe').addEventListener('click', function() {
      _this.sendEvent('unic:teste', 'header', 'clique:acompanhe-inscricao');
    })
  },
  mobile: function() {
    ...
    this.acompanhe.setAttribute('href', 'https://www.vestibulares.com.br/unic/area-candidato/login');
