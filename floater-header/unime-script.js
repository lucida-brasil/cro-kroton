var floaterTeste = {
  ...
  createMarkup: function() {
    ...
    var zap = '<div class="wa-floater-container"><div class="wa-floater-header"><span class="circle"><img alt="" src="data:image/png;base64," alt="Logo unime"></span><div class="txt"><strong>unime</strong><span>Online</span></div></div><div class="wa-floater-messages"><div class="wa-floater-message"><p>Olá!<br>Para prosseguir com o atendimento, vamos precisar de algumas informações:</p></div><div class="wa-floater-fields"><label><span>Qual o seu nome completo?*</span><input type="text" name="nome-completo" id="nome-completo"></label><label><span>Qual o número do seu celular?*</span><input type="text" name="telefone-celular" id="telefone-celular"></label><label><input type="checkbox" name="consentimento" id="consentimento"><span>Eu concordo em receber contato no WhatsApp.</span></label><button>Iniciar Conversa</button></div></div></div>';
    parser = new DOMParser();
  ...
  bind: function() {
    ...
    init.addEventListener('click', function() {
      _this.sendEvent('unime:teste', 'floater:inicio', 'clique:botao');
    ...
    agora.addEventListener('click', function() {
      _this.sendEvent('unime:teste', 'floater:widget-inscreva-se', 'clique:botao');
      _this.hA();
      _this.hM();
      _this.cS('close');
    ...
    ctc.addEventListener('click', function() {
      _this.sendEvent('unime:teste', 'floater:ctc', 'clique:botao');
      _this.hA();
      _this.hM();
      _this.cS('close');
      document.querySelector('iframe.callme').src = 'https://bluecore-kroton-ctc.azurewebsites.net/embed/unime';
    ...
    chatbot.addEventListener('click', function() {
      _this.sendEvent('unime:teste', 'floater:chatbot', 'clique:botao');
      _this.hA();
      _this.hM();
      _this.cS('close');
    ...
    wap.addEventListener('click', function() {
      _this.sendEvent('unime:teste', 'floater:whatsapp', 'clique:botao');
      _this.hA();
      _this.hM();
      _this.cS('close');
    ...
    wapfields.forEach(function(el) {
      this.addEventListener('change', function() {
        _this.sendEvent('unime:teste', 'floater:whatsapp', 'preencheu:' + el.name);
      })
    })

    var wapsend = document.querySelector('.wa-floater-container button');
    wapsend.addEventListener('click', function() {
      _this.sendEvent('unime:teste', 'floater:whatsapp', 'clique:iniciar-conversa');
  ...
  autoStart: function() {
    ...
    setTimeout(function() {
      if (_this.openedOnce == false) {
        _this.sendEvent('unime:teste', 'floater:inicio', 'auto');
...
var headerTeste = {
  ...
  insertButton: function(mob = false) {
    if (mob) {
      ...
      if (document.querySelectorAll('.btn-header-inscreva-se').length === 0) {
        document.querySelector('.unime_ > .second-menu').appendChild(cont);
      }
    } else {
      if (document.querySelectorAll('.btn-header-inscreva-se').length === 0) {
  ...
  bind: function() {
    ...
    document.querySelector('.btn-header-inscreva-se').addEventListener('click', function() {
      _this.sendEvent('unime:teste', 'header', 'clique:inscreva-se');
    })

    document.querySelector('.lnk-header-acompanhe').addEventListener('click', function() {
      _this.sendEvent('unime:teste', 'header', 'clique:acompanhe-inscricao');
    })
  },
  mobile: function() {
    ...
    this.acompanhe.setAttribute('href', 'https://www.vestibulares.com.br/unime/area-candidato/login');
